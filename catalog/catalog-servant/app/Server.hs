...
instance FromHttpApiData Geo where
  parseUrlPiece s =
    ...
    where
      g = readGeo' (s ^. isoString)

instance FromHttpApiData GeoAR where
  parseUrlPiece s =
    maybe (defaultParseError s) Right $ g
    where
      g = readGeoAR (s ^. isoString)
...
type BlazeAPI
  = Capture    "blaze" BlazeHTML :> "archive" :> "collections" :>
...
catalogServer :: FilePath -> Server CatalogAPI
catalogServer mp =
  ( bootstrap
    :<|>
    ( assets'css
